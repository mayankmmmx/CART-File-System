...
int hash(char *str) {
    ...
    unsigned long factor = 3456; //random factor
    ...
    while(*ustr != '\0') { //while the end of the string isn't reached
        hash = hash * factor + *ustr; //creates random location using math
        ustr++; //keep going
    }
...
int32_t cart_poweroff(void) {
    ...
    if(off == -1) { return(-1); } //if shutdown fails, return -1
...
int16_t cart_open(char *path) {
    ...
    if(file_handle == -1) { //if file does not exist create a new one
        ...
        for(i = 1; i < CART_MAX_CARTRIDGES; i++) {
        ...
        file_system.cart_to_use++; //increments cart
...
int32_t cart_read(int16_t fd, void *buf, int32_t count) {
    ...
    response = run_opcode(generate_encoded_opcode(CART_OP_RDFRME, 0, 0, current->data[read_location_frame].frame), &read_in); //gets initial frame
...
int32_t cart_write(int16_t fd, void *buf, int32_t count) {
    ...
    int response, i; //response for cart and iterating var
    if(file_system.files[fd].name[0] == '0' || file_system.files[fd].is_open != 1) { //checks if file exists or is already closed
        return(-1);
    }
    ...
    for(i = 0; i < count; i++) { //iterates through bytes to write
        ...
        if((holding_buffer_count + write_location_bytes + 1) == (CART_FRAME_SIZE - 1) || i == (count - 1)) { //if max data reached or end of file
            ...
            response = run_opcode(generate_encoded_opcode(CART_OP_RDFRME, 0, 0, current->data[write_location_frame].frame), &read_in); //reads frame
            ...
            if(current->current_position % (CART_FRAME_SIZE-1) == 0) { //checks if end of frame
                if(current->data[write_location_frame].frame == CART_CARTRIDGE_SIZE-1) { //if end of cartridge
                    current->data[write_location_frame+1].frame = 0; //set frame to beginning of cartridge
                    current->data[write_location_frame+1].cart = current->data[write_location_frame].cart + 1; //move to next cart
                    if(current->data[write_location_frame+1].cart > file_system.cart_to_use) { //checks if next cartridge isn't already used
                        file_system.cart_to_use = current->data[write_location_frame+1].cart; //sets new cart for file systen
                        file_system.frame_to_use = 1; //sets next frame to use to 1 for file system
                    }
                } else { //if frame isn't full, move to next frame
                    current->data[write_location_frame+1].frame = current->data[write_location_frame].frame+1; //increment frame
                    current->data[write_location_frame+1].cart = current->data[write_location_frame].cart;
                    if(current->data[write_location_frame+1].frame > file_system.frame_to_use) { //checks if new frame is greater than system frame
                        file_system.frame_to_use = current->data[write_location_frame+1].frame + 1; //next frame available for next file
                    }
                }
